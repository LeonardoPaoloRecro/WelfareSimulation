...
const WARS = [
  { label: "World War II", start: 1941, end: 1945, color: "rgba(125, 90, 58, 0.08)" },
  { label: "Korean War", start: 1950, end: 1953, color: "rgba(154, 122, 58, 0.07)" },
  { label: "Vietnam War", start: 1955, end: 1975, color: "rgba(31, 107, 102, 0.06)" },
  { label: "Gulf War", start: 1990, end: 1991, color: "rgba(79, 111, 156, 0.05)" },
  { label: "Iraq War", start: 2003, end: 2011, color: "rgba(176, 109, 79, 0.06)" }
];
...
const RESULT_WINDOW_FIGURES = [
  { id: "war-window-wwii", label: "World War II", start: 1940, end: 1943, includeFull: false },
  { id: "war-window-korea", label: "Korean War", start: 1949, end: 1952, includeFull: false },
  { id: "war-window-vietnam", label: "Vietnam War", start: 1964, end: 1967, includeFull: true }
];

const DERIVED_COLORS = {
  coreWelfare: "#355d5a",
  distributionWelfare: "#7b5867"
};
...
const LEVEL_METRICS = [
  { key: "gdp", label: "Real GDP", color: SERIES_BY_KEY.get("gdp").color, formatter: formatBillions },
  { key: "consumption", label: "Consumption", color: SERIES_BY_KEY.get("consumption").color, formatter: formatBillions },
  { key: "gdpPerCapita", label: "GDP per capita", color: SERIES_BY_KEY.get("gdpPerCapita").color, formatter: formatDollars0 },
  { key: "unemployment", label: "Unemployment rate", color: SERIES_BY_KEY.get("unemployment").color, formatter: formatPercent },
  { key: "gini", label: "Gini index", color: SERIES_BY_KEY.get("gini").color, formatter: formatGini },
  { key: "employment", label: "Employment", color: SERIES_BY_KEY.get("employment").color, formatter: formatThousands },
  { key: "government", label: "Government spending", color: SERIES_BY_KEY.get("government").color, formatter: formatBillions },
  { key: "coreWelfare", label: "Core welfare", color: DERIVED_COLORS.coreWelfare, formatter: formatIndexNumber },
  { key: "distributionWelfare", label: "Full welfare", color: DERIVED_COLORS.distributionWelfare, formatter: formatIndexNumber }
];

const CHANGE_METRICS = [
  { key: "gdpChange", label: "GDP growth", color: SERIES_BY_KEY.get("gdp").color, formatter: formatSignedPercent },
  { key: "consumptionChange", label: "Consumption growth", color: SERIES_BY_KEY.get("consumption").color, formatter: formatSignedPercent },
  { key: "gdpPerCapitaChange", label: "GDP per capita growth", color: SERIES_BY_KEY.get("gdpPerCapita").color, formatter: formatSignedPercent },
  { key: "unemploymentChange", label: "Unemployment change", color: SERIES_BY_KEY.get("unemployment").color, formatter: formatSignedPointChange },
  { key: "giniChange", label: "Gini change", color: SERIES_BY_KEY.get("gini").color, formatter: formatSignedGiniChange },
  { key: "employmentChange", label: "Employment growth", color: SERIES_BY_KEY.get("employment").color, formatter: formatSignedPercent },
  { key: "governmentChange", label: "Government growth", color: SERIES_BY_KEY.get("government").color, formatter: formatSignedPercent },
  { key: "coreWelfareChange", label: "Core welfare change", color: DERIVED_COLORS.coreWelfare, formatter: formatSignedIndexNumber },
  { key: "distributionWelfareChange", label: "Full welfare change", color: DERIVED_COLORS.distributionWelfare, formatter: formatSignedIndexNumber }
];
...
const state = {
  ...
  chartHeight: IS_TOUCH_DEVICE ? 540 : 620,
  relationMode: "change",
...
const PLOT_CONFIG = {
  ...
  showTips: false,
  displayModeBar: IS_TOUCH_DEVICE ? false : "hover",
  doubleClick: "reset",
  modeBarButtonsToRemove: ["lasso2d", "select2d", "zoomIn2d", "zoomOut2d", "hoverClosestCartesian", "hoverCompareCartesian", "toggleSpikelines"]
};
...
function updateHeroForView() {
  ...
  if (state.activeView === "us-case") {
    ...
    eyebrow.textContent = "REAL CASE STUDY";
...
function updateSelectionSummary() {
  ...
  summary.push("Zoom one time-series chart and the other time-series charts will follow the same range.");
...
function renderUnitChart() {
  ...
  const layout = buildMultiRowLayout(panels, {
    height: Math.max(560, panels.length * 240 + 160),
    showZeroLine: false
  });
...
function renderChangeChart() {
  ...
  const layout = buildMultiRowLayout(panels, {
    height: Math.max(600, panels.length * 270 + 160),
    showZeroLine: true
  });
...
function renderWelfareChart() {
  ...
  const traces = chartSeries.map((series) => {
...
function renderMultiplierChart() {
  ...
  const traces = [
    {
      ...
      text: WAR_MULTIPLIER_DATA.map((item) => formatMultiplier(item.outputMultiplier)),
    ...
    },
    {
      ...
      text: WAR_MULTIPLIER_DATA.map((item) => formatMultiplier(item.coreWelfareScaled)),
      textposition: "outside",
      textfont: {
        family: "Arial, sans-serif",
        size: 12,
    ...
    },
    {
      ...
      text: WAR_MULTIPLIER_DATA.map((item) => item.distributionWelfareScaled === null ? "" : formatMultiplier(item.distributionWelfareScaled)),
      textposition: "outside",
      textfont: {
        family: "Arial, sans-serif",
        size: 12,
  ...
  const layout = {
    ...
    height: responsiveHeight(440, 380),
    font: {
      family: "Arial, sans-serif",
      color: "#101518",
      size: 12
    ...
    xaxis: {
      ...
      showgrid: false,
      zeroline: false,
      tickfont: { family: "Arial, sans-serif", size: 12, color: "#49545e" },
      automargin: true
    },
    yaxis: {
      title: {
        text: "Comparison value",
        font: { family: "Arial, sans-serif", size: 13, color: "#101518" }
      },
      showgrid: true,
    ...
    annotations: [
      {
        ...
        x: 0,
        y: 1.03,
        text: "Red = dY/dG, blue = scaled core comparison, light blue = scaled full comparison",
        showarrow: false,
        xanchor: "left",
        font: { family: "Arial, sans-serif", size: 12, color: "#5f686f" }
      }
    ]
  };
...
function renderBenchmarkDecompositionChart() {
  ...
  const decompositionTraces = [
    {
      ...
      text: WAR_MULTIPLIER_DATA.map((item) => formatSignedIndexNumber(item.consumptionContribution)),
      textposition: "outside",
      cliponaxis: false,
      hovertemplate: "<b>%{x}</b><br>Worker consumption contribution: %{y:.4f}<extra></extra>"
    },
    {
      type: "bar",
      x,
      ...
      text: WAR_MULTIPLIER_DATA.map((item) => formatSignedIndexNumber(item.accessContribution)),
      textposition: "outside",
      cliponaxis: false,
      hovertemplate: "<b>%{x}</b><br>Labor-market access contribution: %{y:.4f}<extra></extra>"
    },
    {
      type: "bar",
      x,
      ...
      marker: { color: DERIVED_COLORS.distributionWelfare },
      text: WAR_MULTIPLIER_DATA.map((item) => item.inequalityContribution === null ? "" : formatSignedIndexNumber(item.inequalityContribution)),
      textposition: "outside",
      cliponaxis: false,
      hovertemplate:
        "<b>%{x}</b><br>Inequality contribution: %{y:.4f}<extra></extra>"
    }
  ];
  ...
  const layout = {
    ...
    xaxis: {
      showgrid: false,
      zeroline: false,
      tickfont: { size: 12, color: "#57636f" },
      automargin: true
    },
    yaxis: {
      title: { text: "Contribution to welfare change", font: { size: 12 } },
      showgrid: true,
      gridcolor: "rgba(16, 21, 24, 0.08)",
      zeroline: true,
      zerolinecolor: "rgba(16, 21, 24, 0.16)",
      tickfont: { size: 11, color: "#57636f" },
      automargin: true
    },
    annotations: [
      {
        ...
        xanchor: "right",
        showarrow: false,
        text: "WWII is driven mainly by labor-market access; the inequality term appears only where Gini exists.",
        font: { size: 12, color: "#5f686f" }
      }
    ]
  };
...
function renderWarWindowChart(windowSpec) {
  ...
  const traces = [
    {
      ...
      name: "Real GDP",
      line: { color: SERIES_BY_KEY.get("gdp").color, width: 2.8 },
      marker: { size: 8, color: SERIES_BY_KEY.get("gdp").color },
    ...
    },
    {
      ...
      line: { color: SERIES_BY_KEY.get("consumption").color, width: 2.8 },
      marker: { size: 8, color: SERIES_BY_KEY.get("consumption").color },
    ...
    },
    {
      ...
      line: { color: SERIES_BY_KEY.get("government").color, width: 2.8 },
      marker: { size: 8, color: SERIES_BY_KEY.get("government").color },
    ...
    },
    {
      ...
      name: "Core welfare",
      line: { color: DERIVED_COLORS.coreWelfare, width: 2.8 },
      marker: { size: 8, color: DERIVED_COLORS.coreWelfare },
  ...
  if (windowSpec.includeFull && baseFull !== null) {
    traces.push({
      ...
      name: "Full welfare",
      line: { color: DERIVED_COLORS.distributionWelfare, width: 2.6, dash: "dot" },
      marker: { size: 7, color: DERIVED_COLORS.distributionWelfare },
  ...
  const layout = {
    height: responsiveHeight(500, 420),
    paper_bgcolor: "rgba(0,0,0,0)",
    plot_bgcolor: "rgba(255,255,255,0.88)",
    margin: { l: 88, r: 26, t: 18, b: 54 },
    font: {
      family: "Space Grotesk, sans-serif",
      color: "#101518"
    },
    ...
    yaxis: {
      title: { text: "Index (window start = 100)", font: { size: 12 } },
      showgrid: true,
      gridcolor: "rgba(16, 21, 24, 0.08)",
      zeroline: true,
      zerolinecolor: "rgba(16, 21, 24, 0.16)",
    ...
    annotations: [
      {
        ...
        xanchor: "right",
        showarrow: false,
        text: "All series indexed to the first year of the window",
        font: { size: 12, color: "#5f686f" }
      }
    ]
  };
...
function renderRelationChart() {
  ...
  const trace = {
    ...
    marker: {
      size: 9,
      color: points.map((point) => point.year),
      colorscale: [
        [0, "#1f6b66"],
        [0.5, "#4f6f9c"],
        [1, "#b06d4f"]
    ...
    annotations: [
      {
        ...
        showarrow: false,
        xanchor: "right",
        text: `r = ${regression.correlation.toFixed(2)} • n = ${points.length}`,
        font: { size: 12, color: "#57636f" }
      }
    ]
  };
...
function renderGdpPerCapitaTrendChart() {
  ...
  layout.annotations = [
    ...(layout.annotations || []),
    {
      ...
      xanchor: "right",
      showarrow: false,
      text: "Long-run benchmark: 2% real growth per person per year",
      font: { size: 12, color: "#57636f" }
    }
  ];
...
function renderLongRunChart() {
  ...
  const panels = [
    { key: "gdpPerCapita", title: "GDP pc" },
    { key: "coreWelfareFromBase", title: "Core welfare" },
    { key: "unemploymentPercent", title: "Unemp. (%)" },
    { key: "government", title: "Gov. spending" }
  ];
  ...
  const traces = [
    {
      ...
      mode: "lines",
      x: YEARS,
      y: ENRICHED_DATA.map((row) => row.gdpPerCapita),
      xaxis: "x",
      yaxis: "y",
      name: "GDP per capita",
      line: { color: BENCHMARK_COLORS.core, width: 3.1 },
    ...
    },
    {
      ...
      mode: "lines",
      x: YEARS,
      y: ENRICHED_DATA.map((row) => row.coreWelfare - baseCore),
      xaxis: "x2",
      yaxis: "y2",
      name: "Core welfare",
      line: { color: DERIVED_COLORS.coreWelfare, width: 3.1 },
    ...
    },
    {
      ...
      mode: "lines",
      x: YEARS,
      y: ENRICHED_DATA.map((row) => row.unemployment * 100),
      xaxis: "x3",
      yaxis: "y3",
      name: "Unemployment",
      line: { color: BENCHMARK_COLORS.output, width: 3 },
    ...
    },
    {
      ...
      mode: "lines",
      x: YEARS,
      y: ENRICHED_DATA.map((row) => row.government),
      xaxis: "x4",
      yaxis: "y4",
      name: "Government spending",
      line: { color: SERIES_BY_KEY.get("government").color, width: 3.1 },
  ...
  const layout = buildMultiRowLayout(panels, {
    height: responsiveHeight(1020, 860),
    showZeroLine: false
  });
  ...
  layout.annotations = [
    ...(layout.annotations || []),
    {
      ...
      xanchor: "right",
      showarrow: false,
      text: "Core welfare rebased to 1929 = 0",
      font: { size: 12, color: "#57636f" }
    }
  ];
...
function renderSimulationCharts(result) {
  ...
  const comparisonTrace = {
    ...
    text: [formatMultiplier(result.outputMultiplier)],
    textposition: "outside",
    cliponaxis: false,
  ...
  const comparisonCoreTrace = {
    ...
    text: [formatMultiplier(result.coreMultiplier)],
  ...
  const comparisonFullTrace = {
    ...
    text: [formatMultiplier(result.fullMultiplier)],
  ...
  const comparisonLayout = {
    ...
    yaxis: {
      title: { text: "Output response dY/dG" },
      showgrid: true,
      gridcolor: "rgba(16, 21, 24, 0.08)",
      zeroline: true,
      zerolinecolor: "rgba(16, 21, 24, 0.16)",
    ...
    yaxis2: {
      title: { text: "Welfare response dW/dG" },
      overlaying: "y",
      side: "right",
      showgrid: false,
      zeroline: false,
  ...
  Plotly.react("simulation-comparison-chart", [comparisonTrace, comparisonCoreTrace, comparisonFullTrace], comparisonLayout, PLOT_CONFIG);
  ...
  const decompositionTrace = {
    ...
    marker: {
      color: [
        ...
        DERIVED_COLORS.distributionWelfare
      ]
    },
    text: [result.decomposition.consumption, result.decomposition.access, result.decomposition.inequality].map(formatSignedIndexNumber),
    textposition: "outside",
  ...
  const decompositionLayout = {
    ...
    yaxis: {
      title: { text: "Contribution to welfare change" },
      showgrid: true,
      gridcolor: "rgba(16, 21, 24, 0.08)",
      zeroline: true,
      zerolinecolor: "rgba(16, 21, 24, 0.16)"
...
function buildSingleChartLayout(options) {
  return {
    ...
    yaxis: {
      title: { text: options.yTitle, font: { size: 12 } },
      showgrid: true,
      gridcolor: "rgba(16, 21, 24, 0.08)",
      zeroline: true,
...
function buildMultiRowLayout(panels, options) {
  ...
  panels.forEach((panel, index) => {
    ...
    layout[xAxisKey] = {
      ...
      linecolor: "rgba(16, 21, 24, 0.16)",
      tickfont: { size: 11, color: "#57636f" },
      title: row === panels.length ? { text: "Year", font: { size: 12 } } : undefined,
