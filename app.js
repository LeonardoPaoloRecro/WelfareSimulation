...
const YEARS = window.MACRO_DATA.map((row) => row.year);
const MIN_YEAR = YEARS[0];
const MAX_YEAR = YEARS[YEARS.length - 1];

const ENRICHED_DATA = buildEnrichedData(window.MACRO_DATA);
...
function updateHeroForView() {
  ...
  if (state.activeView === "us-case") {
    ...
    eyebrow.textContent = "Verified U.S. series • deployable on GitHub";
  ...
  if (state.activeView === "simulation") {
    ...
    eyebrow.textContent = "Manual simulation lab";
  ...
  hero.classList.add("is-compact");
  eyebrow.textContent = "Method in plain language";
  description.hidden = true;
  notes.hidden = true;
}
...
function exportVisibleCsv() {
  ...
  link.href = url;
  link.download = `us-macro-visible-${Math.round(state.xRange[0] + 0.5)}-${Math.round(state.xRange[1] - 0.5)}.csv`;
  link.click();
