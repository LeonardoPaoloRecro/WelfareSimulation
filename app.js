...
const LEVEL_PANELS = [
  { key: "billions", title: "Billions of chained 2017 dollars" },
  { key: "dollars", title: "Real GDP per capita, dollars" },
  { key: "thousands", title: "Employment, thousands of people" },
  { key: "percent", title: "Unemployment rate, %" },
  { key: "gini", title: "Gini index" }
];

const CHANGE_PANELS = [
  { key: "growth", title: "Annual growth rate, %" },
  { key: "unemployment", title: "Unemployment change, percentage points" },
  { key: "gini", title: "Gini change, index points" }
];

const WARS = [
  { label: "World War II", start: 1941, end: 1945, color: "rgba(194, 65, 12, 0.10)" },
  { label: "Korean War", start: 1950, end: 1953, color: "rgba(180, 83, 9, 0.10)" },
  { label: "Vietnam War", start: 1955, end: 1975, color: "rgba(15, 118, 110, 0.09)" },
  { label: "Gulf War", start: 1990, end: 1991, color: "rgba(37, 99, 235, 0.08)" },
  { label: "Iraq War", start: 2003, end: 2011, color: "rgba(139, 94, 52, 0.10)" }
];

const WAR_MULTIPLIER_WINDOWS = [
  { label: "WWII onset", startPrev: 1940, end: 1943 },
  { label: "Korea onset", startPrev: 1949, end: 1952 },
  { label: "Vietnam onset", startPrev: 1964, end: 1967 }
];

const DERIVED_COLORS = {
  coreWelfare: "#0b5a55",
  distributionWelfare: "#7c2d12"
};
...
const SIM_FIELDS = [
  {
    key: "year",
    label: "Year",
    note: "Any year label you want to compare.",
    step: "1"
  },
  {
    key: "population",
  ...
  },
  {
    key: "gdp",
    label: "Real GDP",
    note: "Billions of chained dollars.",
    step: "0.01"
  },
  {
    key: "gdpPerCapita",
    label: "GDP per capita",
    note: "Dollars, optional but useful for checks.",
    step: "0.01"
  },
  {
    key: "consumption",
...
const FORMULA_FOCUS_COPY = {
  ...
  access: "This part captures labor-market access. If unemployment rises, the measure falls because a smaller share of the labor force is actually working.",
  distribution: "This part captures distribution. If gains are more concentrated, this term falls and welfare falls with it."
};
...
function renderInsightCards() {
  ...
  document.getElementById("insight-ww2").textContent =
    `Output ${formatMultiplier(ww2.outputMultiplier)} vs core welfare ${formatMultiplier(ww2.coreWelfareMultiplier)}`;
}
...
function renderUnitChart() {
  ...
  const layout = buildMultiRowLayout(panels, {
    height: Math.max(440, panels.length * 190 + 120),
    showZeroLine: false
  });
...
function renderChangeChart() {
  ...
  const layout = buildMultiRowLayout(panels, {
    height: Math.max(480, panels.length * 220 + 120),
    showZeroLine: true
  });
...
function renderMultiplierChart() {
  ...
  const traces = [
    {
      ...
      marker: { color: SERIES_BY_KEY.get("government").color }
    },
    {
      type: "bar",
      x,
      y: WAR_MULTIPLIER_DATA.map((item) => item.coreWelfareMultiplier),
      name: "Core welfare multiplier",
      marker: { color: DERIVED_COLORS.coreWelfare }
    },
    {
      type: "bar",
      x,
      y: WAR_MULTIPLIER_DATA.map((item) => item.distributionWelfareMultiplier),
      name: "Full welfare multiplier",
  ...
  Plotly.react("multiplier-chart", traces, layout, PLOT_CONFIG);
}
...
function renderDatasetTable() {
  ...
  rows.forEach((row) => {
    ...
    tr.innerHTML = `
      <td>${row.year}</td>
      <td>${formatBillions(row.gdp)}</td>
      <td>${formatSignedPercent(row.gdpChange)}</td>
      <td>${formatBillions(row.consumption)}</td>
      <td>${formatSignedPercent(row.consumptionChange)}</td>
      <td>${formatDollars0(row.gdpPerCapita)}</td>
      <td>${formatSignedPercent(row.gdpPerCapitaChange)}</td>
      <td>${formatPercent(row.unemployment)}</td>
      <td>${formatSignedPointChange(row.unemploymentChange)}</td>
      <td>${formatGini(row.gini)}</td>
      <td>${formatSignedGiniChange(row.giniChange)}</td>
      <td>${formatThousands(row.employment)}</td>
      <td>${formatSignedPercent(row.employmentChange)}</td>
      <td>${formatBillions(row.government)}</td>
      <td>${formatSignedPercent(row.governmentChange)}</td>
      <td>${formatIndexNumber(row.coreWelfare)}</td>
      <td>${formatIndexNumber(row.distributionWelfare)}</td>
    `;
    tbody.appendChild(tr);
  });

  document.getElementById("dataset-meta").textContent =
    `${rows.length} rows shown in the current year window${state.tableQuery ? ` and filtered by “${state.tableQuery}”.` : "."}`;
}
...
function clearSimulation() {
  ...
  document.getElementById("simulation-status").textContent =
    "Fields cleared. Enter two observations and run the calculation again.";
...
function calculateSimulation(prev, next) {
  ...
  const coreMultiplier = shock !== 0 ? coreChange / shock : null;
  const fullChange =
    next.distributionWelfare !== null && prev.distributionWelfare !== null
      ? next.distributionWelfare - prev.distributionWelfare
      : null;
  const fullMultiplier = shock !== 0 && fullChange !== null ? fullChange / shock : null;
...
function updateSimulationCards(result) {
  const values = result
    ? {
        ...
        coreNote: "Change in core welfare divided by the fiscal shock share.",
        fullMultiplier: formatMultiplier(result.fullMultiplier),
        fullNote: result.fullMultiplier === null ? "Needs Gini in both years." : "Change in full welfare divided by the fiscal shock share.",
    ...
    : {
        ...
        coreMultiplier: "—",
        coreNote: "Uses worker consumption and employment access only.",
        fullMultiplier: "—",
        fullNote: "Adds the inequality adjustment when both Gini values are present.",
        coreBase: "—",
...
function renderSimulationCharts(result) {
  ...
  const comparisonTrace = {
    type: "bar",
    x: ["Output", "Core welfare", "Full welfare"],
    y: [result.outputMultiplier, result.coreMultiplier, result.fullMultiplier],
    marker: {
      color: [SERIES_BY_KEY.get("government").color, DERIVED_COLORS.coreWelfare, DERIVED_COLORS.distributionWelfare]
    },
    text: [result.outputMultiplier, result.coreMultiplier, result.fullMultiplier].map(formatMultiplier),
  ...
  Plotly.react("simulation-comparison-chart", [comparisonTrace], comparisonLayout, PLOT_CONFIG);
...
function calculateWarWindowStats(windowSpec) {
  ...
  const coreWelfareMultiplier = coreChange / (deltaG / start.gdp);
  const distributionWelfareMultiplier =
  ...
  return {
    ...
    outputMultiplier,
    coreWelfareMultiplier,
    distributionWelfareMultiplier
  };
}
